; Includedatei: "normsource-includes/music-tracker/macros-pt3.i"
; Datum:        4.6.2023
; Version:      1.0
...
;--> E9 "Retrig Note" or ED "Note Delay" <--
  ...
  IFEQ pt_metronome
    ...
    move.b  d0,pt_MetroChannel(a3) ;Zero =  No metronome channel, enable metronome channel with value 1,2,3 or 4
  ENDC
...
;--> 5xy "Tone Portamento + Volume Slide" <--
  IFNE pt_usedfx&pt_cmdbittoneportvolslide
    PT3_EFFECT_TONE_PORTAMENTO_VOLUME_SLIDE
  ENDC
...
;--> 6xy "Vibrato + Volume Slide" <--
  IFNE pt_usedfx&pt_cmdbitvibratovolslide
    PT3_EFFECT_VIBRATO_VOLUME_SLIDE
  ENDC
...
pt_CheckMetronome
    ...
    moveq   #TRUE,d0         ;Needed for longword access
...
pt_NoSampleStart
  ...
  moveq   #1,d2              ;1 word length
  move.w  d2,n_replen(a2)    ;Set repeat length
...
pt_MoreExtCommands
  IFNE pt_usedefx
    move.b  n_cmdlo(a2),d0     ;Get channel extended effect command number
    lsr.b   #NIBBLESHIFTBITS,d0 ;Shift extended effect command number to lower nibble
  ENDC
;--> E0x "Set Filter" <--
...
;--> E3x "Set Glissando Control" <--
  IFNE pt_usedefx&pt_ecmdbitsetglisscontrol
    PT3_EFFECT_SET_GLISSANDO_CONTROL
  ENDC
 
;--> E4x "Set Vibrato Waveform" <--
  IFNE pt_usedefx&pt_ecmdbitsetvibwaveform
    PT3_EFFECT_SET_VIBRATO_WAVEFORM
  ENDC 
...
;--> E7x "Set Tremolo Waveform" <--
  IFNE pt_usedefx&pt_ecmdbitsettrewaveform
    PT3_EFFECT_SET_TREMOLO_WAVEFORM
  ENDC 
...
;--> EAx "Fine Volume Slide Up" <--
  IFNE pt_usedefx&pt_ecmdbitfinevolslideup
    PT3_EFFECT_FINE_VOLUME_SLIDE_UP
  ENDC 

;--> EBy "Fine Volume Slide Down" <--
  IFNE pt_usedefx&pt_ecmdbitfinevolslidedown
    PT3_EFFECT_FINE_VOLUME_SLIDE_DOWN
  ENDC 
...
PT3_EFFECT_TONE_PORTAMENTO_VOLUME_SLIDE MACRO
  CNOP 0,4
...
PT3_EFFECT_VIBRATO_VOLUME_SLIDE MACRO
  CNOP 0,4
...
PT3_EFFECT_SET_GLISSANDO_CONTROL MACRO
  CNOP 0,4
...
PT3_EFFECT_SET_VIBRATO_WAVEFORM MACRO
...
PT3_EFFECT_SET_TREMOLO_WAVEFORM MACRO
...
pt_KarplusStrong
  ...
  move.l  a0,-(a7)
  MULUF.W 2,d7               ;*2 = repeat length in bytes
...
PT3_EFFECT_FINE_VOLUME_SLIDE_UP MACRO
  CNOP 0,4
...
PT3_EFFECT_FINE_VOLUME_SLIDE_DOWN MACRO
  CNOP 0,4
